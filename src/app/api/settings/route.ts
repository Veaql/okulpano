...
async function readExtraVisibilitySettings(schoolId: string) {
  const rows = await prisma.$queryRawUnsafe<Array<{ showWeather: number | boolean | null; showWidget: number | boolean | null }>>(
...
export async function GET() {
  try {
    ...
    const extraVisibility = school.settings ? await readExtraVisibilitySettings(school.id) : {
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (safeSettings) {
      const { showWeather, showWidget, ...prismaSafeSettings } = safeSettings as Record<string, unknown>
      ...
      if (typeof showWeather === "boolean" || typeof showWidget === "boolean") {
